...
window.onload = () => {
  console.log("on load");
  const galleries = Array.from(document.getElementsByClassName("wp-block-custom-blocks-piccy-gallery"));
  galleries.forEach(gallery => {
    ...
    if (thumbnails?.[0]) {
      ...
      imagePreview[0].src = thumbnails[0].src;
    }
    thumbnails.forEach(thumbnail => {
      thumbnail.addEventListener("click", () => {
        ...
        const imagePreview = Array.from(gallery.getElementsByClassName("image-preview"));
        imagePreview[0].src = thumbnail.src;
      });
    });
  });
};
